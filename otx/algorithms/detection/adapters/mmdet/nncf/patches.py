...
def wrap_mmdet_head(obj_cls):
    for fn_name in HEADS_TARGETS["fn_names"]:
        if should_wrap(obj_cls, fn_name, HEADS_TARGETS):
            NNCF_PATCHER.patch(obj_cls, no_nncf_trace_wrapper, fn_name)
            # 'onnx_export' method calls 'forward' method which need to be traced
            NNCF_PATCHER.patch(obj_cls, nncf_trace_wrapper, "forward")
...
# for custom defined
NNCF_PATCHER.patch(HEADS, wrap_register_module, "_register_module")
NNCF_PATCHER.patch(BBOX_ASSIGNERS, wrap_register_module, "_register_module")
NNCF_PATCHER.patch(BBOX_SAMPLERS, wrap_register_module, "_register_module")
NNCF_PATCHER.patch(
